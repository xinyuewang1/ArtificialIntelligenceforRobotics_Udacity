...
class Robot:
    ...
    def sense(self, noise=1):
        ...
        if noise == 1:
            orientation = random.gauss(self.orientation, self.bearing_noise)
        Z = []
        for y, x in landmarks:
            Z.append((atan2(y - self.y, x - self.x) - orientation) % (2 * pi))
        return Z

    def move(self, motion, tolerance_orientation=0.001):
        # copy your code from the previous exercise
        ...
        # add noise
        noised_steering = random.gauss(steering, self.steering_noise)
        noised_distance = random.gauss(distance, self.distance_noise)

        beta = noised_distance * tan(noised_steering) / self.length
        ...
        if beta < tolerance_orientation:
            ...
            x = self.x + (cos(self.orientation) * noised_distance)
            y = self.y + (sin(self.orientation) * noised_distance)
